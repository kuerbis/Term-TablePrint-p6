...
Before the output modifications are made (at a copy of the original data). Leading and trailing whitespaces are removed and paces are squashed to a single white-space. In addition, characters of the Unicode property `Other` are removed. The the elements are stringified with `gist`.
